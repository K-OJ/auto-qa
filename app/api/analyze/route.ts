...
function getMockAnalysis(anomaly: Anomaly): string {
  ...
  switch (anomaly.type) {
    ...
    case 'DROP_OVER_50':
      return `**원인 분석:** ${name}의 처방량이 ${rate} 감소한 것은 데이터 오류와 시장 변화 양쪽 모두 가능성이 있습니다. 동일 계열 약품의 신규 경쟁 제품 출시나 보험급여 기준 변경 등 외부 요인도 검토가 필요합니다.\n\n**확인 항목:** 동일 성분 계열 약품의 당월 처방량 변화 추이 및 보험심사평가원 고시 변경 이력을 확인하세요.\n\n**권고 조치:** 시장 변화 요인이 확인되지 않을 경우 데이터 재검증을 진행하고, 3개월 추이를 함께 검토하세요.`
...
export async function POST(req: NextRequest) {
  ...
  return NextResponse.json({ analysis: getMockAnalysis(anomaly) })
}
